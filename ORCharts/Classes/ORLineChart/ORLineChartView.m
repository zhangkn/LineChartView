...
- (id)chartView:(ORLineChartView *)chartView titleForHorizontalAtIndex:(NSInteger)index {
    ...
    return [NSNumber numberWithInteger:index+1].description;//
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        style.layer.borderColor = [[UIColor colorWithRed:231.0f/255.0f green:231.0f/255.0f blue:231.0f/255.0f alpha:1.0f] CGColor];
...
- (void)or_setTitle:(NSAttributedString *)title inset:(CGFloat)inset {
    ...
    [_label sizeToFit];
    _label.frame = CGRectMake(_label.frame.origin.x, _label.frame.origin.y, _label.frame.size.width+5, _label.frame.size.height);

    CGFloat width = _label.bounds.size.width + inset * 2;
//    CGFloat height = _label.bounds.size.height + inset * 2 + 3.78;
    
    
    CGFloat heightmargn = 3.78*0;//3.78
    
    
    CGFloat height = _label.bounds.size.height + inset * 1 + heightmargn;

    self.bounds = CGRectMake(0, 0, width, height);
    
    _label.center = CGPointMake(width / 2.0, (height - heightmargn) / 2.0);
    
    
    
    _backLayer.path = ({
        UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, width, height - heightmargn) cornerRadius:3];
        UIBezierPath *anglePath = [UIBezierPath bezierPath];
        [anglePath moveToPoint:CGPointMake(width / 2.0f, height)];
        [anglePath addLineToPoint:CGPointMake(width / 2.0 - 3.5, height - heightmargn)];
        [anglePath addLineToPoint:CGPointMake(width / 2.0 + 3.5, height - heightmargn)];
...
//    _bgLineLayer.strokeColor = [UIColor colorWithRed:229 green:229 blue:229 alpha:1].CGColor;
...
//        _bgLineLayer.backgroundColor = [UIColor colorWithRed:229 green:229 blue:229 alpha:1].CGColor;
...
- (void)_or_layoutSubviews {
    ...
    CGFloat topHeight = indecaterHeight * 1;
...
//    self.bgLineLayer.zPosition = 1;
//
//    _lineLayer.zPosition = 10000;
//    self.indicator.layer.zPosition =10001;
